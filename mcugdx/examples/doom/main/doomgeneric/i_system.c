...
void I_AtExit(atexit_func_t func, boolean run_on_error)
{
    ...
    entry = mcugdx_mem_alloc(sizeof(*entry), MCUGDX_MEM_EXTERNAL);
...
static byte *AutoAllocMemory(int *size, int default_ram, int min_ram)
{
    ...
    while (zonemem == NULL)
    {
        ...
        *size = default_ram * 1024 * 1024;
        ...
        zonemem = mcugdx_mem_alloc(*size, MCUGDX_MEM_EXTERNAL);
...
static char *EscapeShellString(char *string)
{
    ...
    result = mcugdx_mem_alloc(strlen(string) * 2 + 3, MCUGDX_MEM_EXTERNAL);
    r = result;
